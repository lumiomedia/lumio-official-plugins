...
function XtreamAccounts({ nav, tt, locale, onReimport, phone = false }: { nav: TvNav; tt: TT; locale: string; onReimport: (list: LiveTvList) => void; phone?: boolean }) {
  ...
  return (
    <>
      <Heading phone={phone}>{tt('xtreamAccount')}</Heading>
      {logins.map((login) => (
        <XtreamAccountCard key={login.id} login={login} tt={tt} locale={locale} phone={phone} onCategories={() => openCategories(login)} />
      ))}
      {pickerLogin ? (
        <TvCategoryPicker
          nav={nav}
...
function XtreamAccountCard({ login, tt, locale, onCategories, phone = false }: { login: XtreamLogin; tt: TT; locale: string; onCategories: () => void; phone?: boolean }) {
  ...
  return (
    ...
    >
      <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column', gap: phone ? 4 : dp(4) }}>
        ...
        <span style={{ fontSize: phone ? 13 : dp(16), color: failed ? '#fca5a5' : phone ? MT.dim : TV.dim }}>{meta}</span>
      </div>
      {phone ? (
