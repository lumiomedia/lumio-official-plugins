...
import { ChannelArt, Icons, PHONE_HIT_MIN_DP, RoundBtn, Tag, TV, Toggle, dp, phoneHitFloor, phoneTextFloor, station } from './tv-ui'
    ...
    return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: dp(48) }}>
        <div
          ...
          style={{ height: dp(phoneHitFloor(52, phone)), minHeight: dp(phoneHitFloor(52, phone)), padding: `0 ${dp(24)}px`, borderRadius: 999, background: TV.s10, display: 'inline-flex', alignItems: 'center', gap: dp(10), fontSize: dp(phoneTextFloor(19, phone)), color: TV.dim, cursor: 'pointer' }}
        >
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, borderRight: `1px solid ${TV.line}`, padding: `${dp(34)}px ${dp(40)}px 0 ${dp(48)}px`, display: 'flex', flexDirection: 'column' }}>
        ...
        <div data-scroll="" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
          ...
          {rows.map((row, index) => {
            ...
            const tag = k === 'now'
              ? <Tag variant="live" phone={phone}>{tt('live')}</Tag>
              : k === 'past'
                ? (rowCanReplay ? <span style={{ fontSize: dp(phoneTextFloor(15, phone)), letterSpacing: '0.08em', textTransform: 'uppercase', color: 'rgba(243,244,248,0.45)' }}>{tt('replay')}</span> : null)
                : <span style={{ fontSize: dp(phoneTextFloor(15, phone)), letterSpacing: '0.08em', textTransform: 'uppercase', color: isReminded(channel, row.p) ? TV.accText : 'rgba(243,244,248,0.45)' }}>{tt('remind')}</span>
            return (
              <div key={row.p.start}>
                {showHeader ? <div style={{ height: dp(44), display: 'flex', alignItems: 'center', paddingLeft: dp(96), fontSize: dp(phoneTextFloor(15, phone)), fontWeight: 600, color: TV.accText }}>{row.day === 'yesterday' ? tt('yesterday') : dayLabel(dayOffset).top}</div> : null}
                <div
                  {...station(primary, undefined, isSelected ? { 'data-init': '' } : undefined)}
                  onFocus={() => setSelectedStart(row.p.start)}
                  style={{ height: dp(phoneHitFloor(66, phone)), minHeight: dp(phoneHitFloor(66, phone)), borderRadius: dp(12), padding: `0 ${dp(16)}px`, display: 'flex', alignItems: 'center', gap: dp(16), background: isSelected ? TV.s10 : 'transparent', color: k === 'past' ? 'rgba(243,244,248,0.55)' : TV.text, cursor: 'pointer' }}
                >
                  <span style={{ width: dp(80), fontSize: dp(phoneTextFloor(21, phone)), fontVariantNumeric: 'tabular-nums' }}>{formatClock(row.p.start, locale)}</span>
                  <span style={{ flex: 1, minWidth: 0, fontSize: dp(phoneTextFloor(21, phone)), fontWeight: k === 'now' ? 600 : 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.p.title}</span>
                  {tag}
                </div>
              </div>
            )
      ...
      <div data-testid="detail" style={{ width: dp(560), flexShrink: 0, padding: `${dp(34)}px ${dp(48)}px ${dp(32)}px ${dp(36)}px`, display: 'flex', flexDirection: 'column', gap: dp(16) }}>
        ...
        {selected?.description ? <div style={{ fontSize: dp(phoneTextFloor(18, phone)), color: 'rgba(243,244,248,0.75)', lineHeight: 1.4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 5, WebkitBoxOrient: 'vertical' }}>{selected.description}</div> : null}
        <div style={{ display: 'flex', gap: dp(12) }}>
          <div
            data-testid="primary-action"
            {...station(primary, undefined, rows.length === 0 ? { 'data-init': '' } : undefined)}
            style={{ height: dp(phoneHitFloor(52, phone)), minHeight: dp(phoneHitFloor(52, phone)), padding: `0 ${dp(24)}px`, borderRadius: 999, background: TV.acc, color: TV.onAcc, display: 'inline-flex', alignItems: 'center', gap: dp(10), fontSize: dp(phoneTextFloor(19, phone)), fontWeight: 600, cursor: 'pointer' }}
          >
        ...
        <div style={{ marginTop: 'auto', padding: `${dp(18)}px ${dp(20)}px`, borderRadius: dp(14), background: TV.s07, border: `1px solid ${TV.lineCard}`, display: 'flex', flexDirection: 'column', gap: dp(10) }}>
          <div style={{ fontFamily: TV.mono, fontSize: dp(phoneTextFloor(12, phone)), letterSpacing: '0.14em', color: TV.accText }}>{tt('channelInfo')}</div>
          {[
            ...
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: dp(phoneTextFloor(17, phone)) }}><span style={{ color: TV.muted }}>{label}</span><span>{value}</span></div>
          ))}
          {lockAvailable ? (
            <div {...station(requestLockToggle)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: dp(phoneTextFloor(17, phone)), paddingTop: dp(10), borderTop: `1px solid ${TV.line}`, minHeight: phone ? dp(PHONE_HIT_MIN_DP) : undefined, cursor: 'pointer' }}>
