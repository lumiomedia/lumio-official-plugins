...
import { ChannelArt, TV, dp, phoneHitFloor, phoneTextFloor, station } from './tv-ui'
      ...
      <div data-testid="search-query-col" style={queryColStyle}>
        {tvMode ? (
          ...
          <div style={{ height: dp(64), borderRadius: dp(14), background: TV.s10, display: 'flex', alignItems: 'center', padding: `0 ${dp(20)}px`, fontSize: dp(28), color: query ? TV.text : 'rgba(243,244,248,0.5)' }}>
            {query || <span style={{ fontSize: dp(phoneTextFloor(18, phone)) }}>{tt('searchPlaceholder')}</span>}
        ...
        <div data-row="" style={{ display: 'flex', gap: dp(8), overflowX: 'auto', minHeight: dp(44) }}>
          {hints.map((hint) => (
            <div key={hint} data-testid="search-suggestion" {...station(() => setQuery(hint))} style={{ height: dp(phoneHitFloor(44, phone)), minHeight: dp(phoneHitFloor(44, phone)), padding: `0 ${dp(18)}px`, borderRadius: 999, background: TV.s08, display: 'inline-flex', alignItems: 'center', fontSize: dp(phoneTextFloor(18, phone)), whiteSpace: 'nowrap', cursor: 'pointer' }}>{hint}</div>
          ))}
        </div>
      ...
      <div data-live-tv-search-results="" data-scroll="" style={resultsStyle}>
        <section data-testid="search-channels" style={{ display: 'flex', flexDirection: 'column', gap: dp(8) }}>
          ...
          {channels.map((channel) => {
            ...
            return (
              <div key={channelKey(channel)} {...station(() => nav.openChannel(channel), (el) => nav.channelMenu(channel, el))} style={{ height: dp(phoneHitFloor(80, phone)), minHeight: dp(phoneHitFloor(80, phone)), borderRadius: dp(12), display: 'flex', alignItems: 'center', gap: dp(14), padding: `0 ${dp(12)}px`, cursor: 'pointer' }}>
                <span style={{ width: dp(44), fontSize: dp(phoneTextFloor(16, phone)), color: 'rgba(243,244,248,0.5)', textAlign: 'right' }}>{model.channelNumber(channel) ?? ''}</span>
                <ChannelArt channel={channel} style={{ width: dp(76), height: dp(50), flexShrink: 0 }} radius={dp(8)} />
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: dp(phoneTextFloor(20, phone)), fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{channel.name}</div>
                  <div style={{ fontSize: dp(phoneTextFloor(16, phone)), color: 'rgba(243,244,248,0.6)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{info.now ? `${tt('colNow')}: ${info.now.title}` : tt('noProgramme')}</div>
                </div>
                <span style={{ fontSize: dp(phoneTextFloor(15, phone)), color: 'rgba(243,244,248,0.5)' }}>{channel.group}</span>
              </div>
            )
          })}
        </section>
        <section data-testid="search-programmes" style={{ display: 'flex', flexDirection: 'column', gap: dp(6) }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: dp(12) }}><span style={{ fontSize: dp(phoneTextFloor(24, phone)), fontWeight: 600 }}>{tt('searchProgrammes')}</span><span style={{ fontSize: dp(phoneTextFloor(16, phone)), color: 'rgba(243,244,248,0.5)' }}>{tt('hits', { count: programmes.length })}</span></div>
          {/* Sökningen går till appen: en tom lista betyder "hämtar" tills
              svaret kommit, inte "inga träffar". */}
          {query && programmes.length === 0 ? <div data-testid="search-programmes-empty" style={{ color: TV.dim, fontSize: dp(phoneTextFloor(18, phone)) }}>{programmesLoading ? tt('loadingGuide') : tt('noResults')}</div> : null}
          {programmes.map((hit) => (
            <div key={`${channelKey(hit.channel)}:${hit.programme.start}`} {...station(() => nav.openChannel(hit.channel, hit.programme.start))} style={{ height: dp(phoneHitFloor(64, phone)), minHeight: dp(phoneHitFloor(64, phone)), borderRadius: dp(12), display: 'flex', alignItems: 'center', gap: dp(16), padding: `0 ${dp(12)}px`, cursor: 'pointer' }}>
              <span style={{ width: dp(80), fontSize: dp(phoneTextFloor(18, phone)), color: 'rgba(243,244,248,0.6)', fontVariantNumeric: 'tabular-nums' }}>{formatClock(hit.programme.start, locale)}</span>
              <span style={{ flex: 1, minWidth: 0, fontSize: dp(phoneTextFloor(20, phone)), whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{hit.programme.title}</span>
              <span style={{ fontSize: dp(phoneTextFloor(16, phone)), color: 'rgba(243,244,248,0.5)' }}>{hit.channel.name}</span>
            </div>
          ))}
        </section>
      </div>
