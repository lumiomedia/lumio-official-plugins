...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', ((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (raw.includes('get_live_categories')) return json([{ category_id: '1', category_name: 'Sport' }, { category_id: '2', category_name: 'News' }])
    if (raw.includes('player_api.php')) return json({ user_info: { auth: 1, status: 'Active' } })
    if (path === '/api/live-tv/logo-fallback') return json({ matched: 3, total: 9 })
    return json({})
  }) as typeof fetch)
})
...
describe('TV-sidan (handoff §4.2)', () => {
  ...
  it('L:list på Xtream: Server categories öppnar en egen vy med växlar och Apply & fetch; Complete logos finns som rad', async () => {
    const login: liveTvData.XtreamLogin = { id: 'login-1', base: 'http://tv.kkz.test:8080', username: 'u', password: 'p', format: 'ts', categoryIds: [] }
    writePluginJson(LIVE_TV_PLUGIN_ID, 'xtream_logins', [login])
    writePluginJson(LIVE_TV_PLUGIN_ID, 'lists', [list({ id: 'x1', name: 'tv.kkz.test:8080', kind: 'xtream', source: 'xtream://tv.kkz.test:8080/login-1', xtreamLoginId: 'login-1', channelCount: 10, groups: [] })])
    const importSpy = vi.spyOn(liveTvData, 'importList').mockResolvedValue({ state: 'done', received: 10, total: 10, result: { total: 10, groups: [], urlTvg: null, truncated: false } })
    render(<LiveTvSettingsSection />)
    fireEvent.click(row(/^tv\.kkz\.test:8080/))
    const p = panel('tv-playlist-panel')
    const srv = within(p).getByRole('button', { name: /^Server categories/ })
    expect(srv).toHaveTextContent('All categories are fetched')
    fireEvent.click(srv)
    const sc = panel('tv-server-categories-panel')
    expect(await within(sc).findByRole('switch', { name: 'Sport' })).toHaveAttribute('aria-checked', 'true')
    fireEvent.click(within(sc).getByRole('switch', { name: 'News' }))
    fireEvent.click(within(sc).getByRole('button', { name: 'Apply & fetch' }))
    expect(liveTvData.getXtreamLogins()[0].categoryIds).toEqual(['1'])
    await waitFor(() => expect(importSpy).toHaveBeenCalledTimes(1))
    expect(screen.queryByTestId('tv-server-categories-panel')).toBeNull()
    expect(within(panel('tv-playlist-panel')).getByRole('button', { name: /^Server categories/ })).toHaveTextContent('1 of 2 fetched')
    fireEvent.click(within(panel('tv-playlist-panel')).getByRole('button', { name: /^Complete logos/ }))
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('3 of 9 completed'))
  })
